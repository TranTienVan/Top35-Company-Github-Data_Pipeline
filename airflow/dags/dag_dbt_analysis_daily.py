...
# Parse nodes
import json
JSON_MANIFEST_DBT = '/opt/dbt/target/manifest.json'
...
for node in nodes:
    if ('daily' in nodes[node]['tags']):
        ...
        bsh_cmd = 'cd /opt/dbt && dbt run --models {nodeName} --vars \'{{"start_date":"{start_date}", "end_date":"{end_date}"}}\' '.format(nodeName = node, start_date = date_start, end_date = date_end)
